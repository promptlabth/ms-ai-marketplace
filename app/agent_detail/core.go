	...
	func (c *Core) SearchAgentDetails(ctx context.Context, keyword string) (*[]AgentDetailEntity, error) {
		var agentDetail []AgentDetailEntity
		if err := c.db.Where("name LIKE ?", "%"+keyword+"%").Find(&agentDetail).Error; err != nil {
			return nil, err
		}
		return &agentDetail, nil
	}
