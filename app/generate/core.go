...
func (c *Core) Generate(ctx context.Context, prompt string, model string) (string, error) {
	float16Key := os.Getenv("FLOAT16_KEY")
	if float16Key == "" {
		return "", fmt.Errorf("FLOAT16_KEY environment variable not set")
	}
	...
	return "", fmt.Errorf("unexpected response format: %s", string(bodyResp))
}
